...
use quasar_compile::{CompiledTransaction, JsonIR};
...
pub fn resolve_rpc_url(explicit: Option<&str>, network: &str) -> Result<String, SimulationError> {
    ...
    // 2. QUASAR_RPC_URL env var
    if let Ok(url) = std::env::var("QUASAR_RPC_URL") {
        if !url.is_empty() {
            return Ok(url);
        }
    }
    ...
    match network {
        ...
        other => Err(SimulationError::InvalidResponse(format!(
            "no default RPC URL for network '{}'; use --rpc-url or set QUASAR_RPC_URL",
            other
        ))),
    }
}
...
mod tests {
    ...
    fn resolve_rpc_url_network_default_testnet() {
        ...
        std::env::remove_var("QUASAR_RPC_URL");
