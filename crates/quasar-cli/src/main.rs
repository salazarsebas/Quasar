...
use quasar_check::{Diagnostic, Resolver, Severity, TypeCheckError, TypeChecker, Validator};
use quasar_compile::{Compiler, JsonIR, XdrCompiler};
use quasar_exec::{
    AbiImporter, ExecutionOutcome, ExecutionResult, Executor, ExecutorConfig, Simulator,
};
use quasar_syntax::ast::{Call, ConstDecl, ConstValue, Directive, MapEntry, Program, Value};
use quasar_syntax::lexer::Lexer;
use quasar_syntax::parser::Parser;
...
#[derive(ClapParser)]
#[command(name = "quasar", about = "Quasar DSL compiler for Soroban")]
#[command(version)]
struct Cli {
...
enum Commands {
    ...
    Simulate {
        /// Input .soro file
        file: String,
        /// RPC endpoint URL (overrides QUASAR_RPC_URL env and network default)
        #[arg(long)]
...
fn main() {
    ...
    match cli.command {
        ...
        Commands::Simulate {
            ...
            match run_pipeline(&source, &file, c) {
                PipelineResult::Compiled(ir) => {
                    ...
                    let url = match quasar_exec::simulator::resolve_rpc_url(
                        rpc_url.as_deref(),
                        &ir.network,
                    ) {
                        Ok(u) => u,
                    ...
                    if json {
                        ...
                        for result in &results {
                            print!(
                                "{}",
                                quasar_exec::simulator::format_human_result(
                                    result,
                                    results.len(),
        ...
        Commands::Import {
            ...
            let url = match quasar_exec::simulator::resolve_rpc_url(rpc_url.as_deref(), &network) {
                Ok(u) => u,
                Err(e) => {
                    eprintln!("{}{}error{}: {}", c.red, c.bold, c.reset, e);
                    process::exit(1);
        ...
        Commands::Run {
            ...
            match run_pipeline(&source, &file, c) {
                PipelineResult::Compiled(ir) => {
                    ...
                    if let Err(e) = quasar_exec::decode_secret_key(&sk) {
                        eprintln!("{}{}error{}: {}", c.red, c.bold, c.reset, e);
                        process::exit(1);
                    }
...
mod tests {
    ...
    fn version_command() {
        ...
        assert!(stdout.contains("quasar v"), "stdout: {}", stdout);
    }
