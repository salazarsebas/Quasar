...
use quasar_syntax::ast::{ConstDecl, ConstValue, Program, Value};
...
impl Resolver {
    pub fn resolve(program: &mut Program) -> Vec<Diagnostic> {
        ...
        for directive in &program.directives {
            if let quasar_syntax::ast::Directive::Source { value, span } = directive {
...
mod tests {
    use super::*;
    use crate::Severity;
    use quasar_syntax::ast::{Call, ConstDecl, ConstValue, Directive, Program, Value};
    ...
    fn identical_output_with_and_without_consts() {
        ...
        let ir_a = quasar_compile::Compiler::compile(&program_a);
        let ir_b = quasar_compile::Compiler::compile(&program_b);
        assert_eq!(
    ...
    fn resolve_in_map_value() {
        ...
        program.calls[0].args.push(Value::Map(
            vec![quasar_syntax::ast::MapEntry {
